        ...
        TextButton(
          onPressed: () {
            if (_formKey.currentState?.validate() ?? false) {
              final quantityDouble = double.parse(_quantityController.text);
              final unitPrice = double.parse(_rateController.text);
              final taxRate = double.tryParse(_taxRateController.text) ?? 0.0;

              // Calculate total with tax
              final totalBeforeTax = quantityDouble * unitPrice;
              final taxAmount = totalBeforeTax * (taxRate / 100);
              final totalPrice = totalBeforeTax + taxAmount;

              // Create invoice item using the correct format from invoice_item.dart
              ...
              if (selectedItem == null && _isManualEntry) {
                ...
                saveItemDialog.then((shouldSave) {
                  if (shouldSave == true) {
                    context.read<ItemBloc>().add(
                          AddItemEvent(
                            ...
                            hsn: _hsnController.text.isEmpty
                                ? null
                                : _hsnController.text,
                            taxRate: taxRate, // Using the taxRate here
